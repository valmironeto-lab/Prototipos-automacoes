...
class BSM_Cron {
	...
	private function register_hooks() {
		...
		// ALTERAÇÃO: Movido de 'admin_init' para 'init' para garantir que seja executado em mais contextos.
	...
	/**
	 * Processa a fila de automações delegando para o motor de automação.
	 */
	public function process_automation_queue() {
		...
		$items_to_process = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$table_queue} WHERE status = 'waiting' AND process_at <= %s LIMIT 20",
		...
		// Marca os itens como 'processando' para evitar que sejam pegos novamente
	...
	/**
	 * Garante que as tarefas agendadas (cron jobs) estão sempre ativas.
	 *
	 * Esta função verifica se as tarefas do plugin estão agendadas e, caso não estejam,
	 * agenda-as novamente. É uma salvaguarda contra a perda de agendamentos.
	 * O uso de um 'transient' como lock evita que esta verificação pesada seja executada
	 * em todas as requisições, melhorando a performance.
	 */
