...
class BSM_DB {
	...
	public static function create_database_tables() {
		...
		// FASE 1: Atualização da tabela de automações
		$sql_automations = "CREATE TABLE {$wpdb->prefix}bluesendmail_automations ( automation_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT, name varchar(255) NOT NULL, status varchar(20) NOT NULL DEFAULT 'inactive', trigger_type varchar(50) NOT NULL, trigger_settings text DEFAULT NULL, created_at datetime NOT NULL DEFAULT '0000-00-00 00:00:00', PRIMARY KEY  (automation_id), KEY trigger_type (trigger_type), KEY status (status) ) $charset_collate;";
		
		// FASE 3: Adição de parent_id e branch
		$sql_automation_steps = "CREATE TABLE {$wpdb->prefix}bluesendmail_automation_steps ( step_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT, automation_id bigint(20) UNSIGNED NOT NULL, parent_id bigint(20) UNSIGNED NOT NULL DEFAULT 0, branch varchar(10) DEFAULT NULL, step_order int(11) NOT NULL DEFAULT 0, step_type varchar(50) NOT NULL, step_settings text DEFAULT NULL, PRIMARY KEY  (step_id), KEY automation_id (automation_id), KEY parent_id (parent_id) ) $charset_collate;";
        
		$sql_automation_queue = "CREATE TABLE {$wpdb->prefix}bluesendmail_automation_queue ( queue_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT, automation_id bigint(20) UNSIGNED NOT NULL, contact_id bigint(20) UNSIGNED NOT NULL, status varchar(20) NOT NULL DEFAULT 'waiting', current_step_id bigint(20) UNSIGNED DEFAULT 0, process_at datetime NOT NULL DEFAULT '0000-00-00 00:00:00', created_at datetime NOT NULL DEFAULT '0000-00-00 00:00:00', PRIMARY KEY  (queue_id), KEY automation_id (automation_id), KEY contact_id (contact_id), KEY status (status) ) $charset_collate;";
	...
	/**
	 * Roda scripts de migração de uma única vez.
	 */
	private static function run_migration_scripts() {
		$migration_v2_done = get_option( 'bsm_migration_v2_complete', false );
		if ( ! $migration_v2_done ) {
			self::migrate_automations_to_v2_structure();
			update_option( 'bsm_migration_v2_complete', true );
		}
	}

	/**
	 * Migra as automações antigas para a nova estrutura de steps.
	 */
	private static function migrate_automations_to_v2_structure() {
		global $wpdb;
		$automations_table = $wpdb->prefix . 'bluesendmail_automations';

		// Verifica se as colunas antigas existem
		$trigger_meta_exists = $wpdb->get_var( $wpdb->prepare( "SHOW COLUMNS FROM `{$automations_table}` LIKE %s", 'trigger_meta' ) );
		
		if ( ! $trigger_meta_exists ) {
			return; // A migração provavelmente já foi feita ou não é necessária
		}

		$old_automations = $wpdb->get_results( "SELECT automation_id, trigger_meta, action_meta FROM {$automations_table} WHERE trigger_settings IS NULL" );

		foreach ( $old_automations as $automation ) {
			// Atualiza a tabela principal de automações
			$trigger_settings = array( 'list_id' => $automation->trigger_meta );
			$wpdb->update(
				$automations_table,
				array( 'trigger_settings' => maybe_serialize( $trigger_settings ) ),
				array( 'automation_id' => $automation->automation_id )
			);

			// Insere o passo de ação correspondente
			$step_settings = array( 'campaign_id' => $automation->action_meta );
			$wpdb->insert(
				...
				array(
					...
					'step_order'    => 1,
					'step_type'     => 'action',
					'step_settings' => maybe_serialize( $step_settings ),
				)
			);
		}

		// Remove as colunas antigas se a migração foi bem-sucedida
		if ( ! empty( $old_automations ) ) {
			$wpdb->query( "ALTER TABLE {$automations_table} DROP COLUMN trigger_meta, DROP COLUMN action_type, DROP COLUMN action_meta;" );
		}
	}
