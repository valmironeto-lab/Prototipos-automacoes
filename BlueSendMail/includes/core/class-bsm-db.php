...
class BSM_DB {
	...
	public static function create_database_tables() {
        ...
        $sql_automation_queue = "CREATE TABLE {$wpdb->prefix}bluesendmail_automation_queue ( queue_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT, automation_id bigint(20) UNSIGNED NOT NULL, contact_id bigint(20) UNSIGNED NOT NULL, current_step_id bigint(20) UNSIGNED NOT NULL DEFAULT 0, status varchar(20) NOT NULL DEFAULT 'waiting', process_at datetime NOT NULL DEFAULT '0000-00-00 00:00:00', created_at datetime NOT NULL DEFAULT '0000-00-00 00:00:00', PRIMARY KEY  (queue_id), KEY automation_id (automation_id), KEY contact_id (contact_id), KEY status (status) ) $charset_collate;";
		$sql_automation_steps = "CREATE TABLE {$wpdb->prefix}bluesendmail_automation_steps ( step_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT, automation_id bigint(20) UNSIGNED NOT NULL, parent_id bigint(20) UNSIGNED NOT NULL DEFAULT 0, branch varchar(10) DEFAULT NULL, step_order int(11) NOT NULL DEFAULT 0, step_type varchar(50) NOT NULL, step_settings text, PRIMARY KEY  (step_id), KEY automation_id (automation_id), KEY parent_id (parent_id) ) $charset_collate;";
		...
		// Roda a migração uma única vez
		if ( ! get_option( 'bsm_v2_migration_done' ) ) {
			self::run_v2_migration();
			update_option( 'bsm_v2_migration_done', true );
		}
	}
	...
	/**
	 * Roda o script de migração de dados da v1 para a v2 do sistema de automação.
	 */
	private static function run_v2_migration() {
		global $wpdb;
		$table_automations = $wpdb->prefix . 'bluesendmail_automations';

		// 1. Renomeia as colunas antigas se existirem
		$wpdb->query( "ALTER TABLE `{$table_automations}` CHANGE `trigger_meta` `trigger_settings` TEXT" );
		$wpdb->query( "ALTER TABLE `{$table_automations}` DROP COLUMN IF EXISTS `action_type`" );
		$wpdb->query( "ALTER TABLE `{$table_automations}` DROP COLUMN IF EXISTS `action_meta`" );
		
		// 2. Converte os dados das automações antigas
		$old_automations = $wpdb->get_results( "SELECT * FROM {$table_automations} WHERE trigger_settings IS NOT NULL" );
		
		foreach( $old_automations as $automation ) {
			// Verifica se o trigger_settings já é um array serializado
			$settings = @unserialize($automation->trigger_settings);
			if ($settings !== false) {
				continue; // Já está no formato novo
			}

			// Se não for, assume que é o formato antigo (apenas o list_id)
			$list_id = absint($automation->trigger_settings);
			if ($list_id > 0) {
				$new_settings = serialize(array('list_id' => $list_id));
				$wpdb->update(
					$table_automations,
					array('trigger_settings' => $new_settings),
					array('automation_id' => $automation->automation_id)
				);

				// Migra a ação antiga para um novo passo
				$action_meta = $wpdb->get_var($wpdb->prepare("SELECT action_meta FROM {$wpdb->prefix}bluesendmail_automations WHERE automation_id = %d", $automation->automation_id));
				if ($action_meta > 0) {
					$wpdb->insert(
						...
						array(
							...
							'step_order'    => 1,
							'step_type'     => 'action',
							'step_settings' => serialize(array('campaign_id' => $action_meta)),
						)
					);
				}
			}
